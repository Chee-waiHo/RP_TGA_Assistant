...
function createTimeSlot(path, documentId, timeSlotData) {
  ...
  const baseUrl = "https://firestore.googleapis.com/v1beta1/projects/newbot-a9161/databases/(default)/documents/"
  + path + "/" + documentId +"?updateMask.fieldPaths=from&updateMask.fieldPaths=max&updateMask.fieldPaths=to&updateMask.fieldPaths=venue";
...
function uploadEligibleStudents () {
 ...
 logSheet.appendRow(["END: Upload Students Information from  " + sheetName,
 Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "dd-MMM-yyyy HH:mm")]);
 SpreadsheetApp.getUi().alert ("Upload Attempt Completed. Please verify uploaded contents and check logs for errors");
}
...
function uploadTimeslots () {
 ...
 logSheet.appendRow(["END: Upload Time Timeslot Master from  " + sheetName,
 Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "dd-MMM-yyyy HH:mm")]);
 SpreadsheetApp.getUi().alert ("Upload Attempt Completed. Please verify uploaded contents and check logs for errors");
}
